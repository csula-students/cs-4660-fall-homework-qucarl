...
/**
 * Adjacency list is probably the most common implemaentation to store the
 * unknown loose graph
 *
...
public class AdjacencyList implements Representation {
	...
	private HashMap<Node, Collection<Edge>> adjacencyList = new HashMap();

	// @SuppressWarnings({ "unchecked", "rawtypes" })
	// public static void main(String[] args){
	// File file1 = new
	// File("C:/Users/Gang/Desktop/CS4660/cs-4660-fall-homework-qucarl/src/test/resources/homework-1/graph-1.txt");
...
//		adjacencyList = new HashMap();
	...
	public int distance(Node from, Node to) {
		if (!adjacencyList.containsKey(from)) {
			...
			ArrayList<Edge> edge = (ArrayList) adjacencyList.get(from);
			for (int i = 0; i < edge.size(); i++) {
				if (edge.get(i).getTo().equals(to)) {
					return edge.get(i).getValue();
				}
			}
		}
		return 0;
